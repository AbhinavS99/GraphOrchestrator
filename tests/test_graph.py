...
async def test_42_concurrent_aggregators_do_not_interfere():
    @node_action
    def node1(state: State): state.messages.append("A"); return state
    @node_action
    def node2(state: State): state.messages.append("B"); return state
    @aggregator_action
...
async def test_47_processing_node_sync():
    @node_action
    def sync_action(state): state.messages.append("sync"); return state
...
async def test_48_processing_node_async():
    @node_action
    async def async_action(state): await asyncio.sleep(0.01); state.messages.append("async"); return state
...
async def test_50_ai_node_executes():
    class DummyAIAction(AIActionBase):
        def build_model(self): self._model_built = True
        async def process_state(self, state): state.messages.append("ai"); return state
...
async def test_51_aggregator_node_sync():
    @aggregator_action
    def aggregator(states): s = states[0]; s.messages.append("agg"); return s
...
async def test_52_aggregator_node_async():
    @aggregator_action
    async def aggregator(states): await asyncio.sleep(0.01); s = states[0]; s.messages.append("agg"); return s
    node = AggregatorNode("agg", aggregator)
...
async def test_53_async_routing_function():
    @routing_function
    async def router(state): await asyncio.sleep(0.01); return "end"
    builder = GraphBuilder()
    builder.add_node(ProcessingNode("n1", passThrough))
    builder.add_concrete_edge("start", "n1")
...
async def test_54_graph_mixed_nodes():
    @node_action
    def p1(state): state.messages.append("p1"); return state
    @tool_method
    async def t1(state): state.messages.append("t1"); return state
    @node_action
    def p2(state): state.messages.append("p2"); return state
    @aggregator_action
    ...
    for label in {"p1", "t1", "agg", "p2"}:
        assert label in result.messages, f"{label} not found in state: {result.messages}"

@pytest.mark.asyncio
async def test_55_aggregator_single_input_still_works():
    @aggregator_action
    def agg(states): return states[0]
    builder = GraphBuilder()
    builder.add_node(ProcessingNode("solo", passThrough))
    builder.add_aggregator(AggregatorNode("agg", agg))
    builder.add_concrete_edge("start", "solo")
    builder.add_concrete_edge("solo", "agg")
    builder.add_concrete_edge("agg", "end")
    result = await GraphExecutor(builder.build_graph(), State(messages=["ok"])).execute()
...
async def test_68_multiple_conditional_edges():
    ...
    builder.add_conditional_edge("cond_node", ["branch1", "branch2", "branch3"], multi_router)
    builder.add_concrete_edge("branch1", "end")
    builder.add_concrete_edge("branch2", "end")
...
async def test_69_execution_exceeds_max_supersteps():
    ...
    assert "Max supersteps reached" in str(exc_info.value) or "loop" in str(exc_info.value)
